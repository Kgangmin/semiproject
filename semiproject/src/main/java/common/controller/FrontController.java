...
@MultipartConfig(maxFileSize = 20971520, maxRequestSize = 31457280 )
@WebServlet(
		description = "사용자가 웹에서 *.hb 을 했을 경우 이 서블릿이 응답을 해주도록 한다.", 
		urlPatterns = { "*.hb" }, 
		initParams = { 
				@WebInitParam(name = "propertyConfig", value = "C:/git/semiproject/semiproject/semiproject/src/main/webapp/WEB-INF/Command.properties", description = "*.hb 에 대한 클래스의 매핑파일")
		})
public class FrontController extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		// 확인용 //
		//System.out.println("~~~~~확인용 => 서블릿 FrontController의 init메소드가 실행됨" );
		//~~~~~확인용 => 서블릿 FrontController의 init메소드가 실행됨
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// System.out.println("###확인용 => 서블릿 FrontController의 doGet메소드가 실행됨" );
		...
		if(action == null) {
			...
			//>>> /test/avksjdka.up는 URI 패턴에 맵핑된 클래스는 없습니다. <<<
		}
		else {
